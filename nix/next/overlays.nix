...
,
  ...
  tezos-sapling-parameters = final.stdenv.mkDerivation rec {
    pname = "tezos-sapling-parameters";
    version = "1.1.0";
    src = final.fetchurl {
      url = "https://gitlab.com/tezos/tezos-sapling-parameters/-/archive/${version}/tezos-sapling-parameters-${version}.tar.gz";
      sha256 = "sha256-zE+HgPMSu/Sa5hOAhXdEttbVGj6tyO+LNLtyjpjTBhY=";
    };

    installPhase = ''
      mkdir -p $out
      cp ./sapling-output.params $out/sapling-output.params
      cp ./sapling-spend.params $out/sapling-spend.params
    '';
  };
    ...
    builtins.mapAttrs
      (ocamlVersion: curr_ocaml:
        curr_ocaml.overrideScope' (oself: osuper:
          ...
          in
          {
            rlp = osuper.buildOasisPackage rec {
              pname = "rlp";
              version = "0.1";
              ...
              propagatedBuildInputs = with osuper; [ rope hex num ];

              checkInputs = with osuper; [ ounit ];
            };

            ppx_irmin = osuper.ppx_irmin.overrideAttrs (_o: rec {
              version = "3.7.2";
            ...
            octez-libs = oself.callPackage ./octez/libs.nix {
              inherit src;
            ...
            octez-evm-proxy-lib = oself.callPackage ./octez/evm-proxy-lib.nix { };
            octez-injector = oself.callPackage ./octez/octez-injector.nix { };
            octez-smart-rollup = oself.callPackage ./octez/smart-rollup.nix { };
            octez-smart-rollup-node-lib = oself.callPackage ./octez/octez-smart-rollup-node-lib.nix { };
